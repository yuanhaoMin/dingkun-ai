import os
import pandas as pd
from app.model.auto_processor import AutoProcessor
from app.agent.helper_agent import navigate_to_page, answer_documentation
from app.config.environment import get_milvus_collection
from app.config.environment import get_openai_key
from app.db.SessionManager import SessionManager
from app.constant.schema.helper_schemas import csv_system_prompt, function_csv
...
PAGE_NAVIGATION_TEXT = (
    "跳转页面或查询页面相关信息、事故发生前一刻的人员分布图、人员的历史轨迹、人员的最终位置、人员的详细信息、特定人员实时轨迹、在线人员列表、在线车辆列表"
)
...
def load_situations_embeddings():
    ...
    for situation, (text, embedding) in situations_dict.items():
        if situation == "general_query":
            assert (
                text == GENERAL_QUERY_TEXT
            ), "Text does not match the expected text for general_query"
        elif situation == "data_analysis":
            assert (
                text == DATA_ANALYSIS_TEXT
            ), "Text does not match the expected text for data_analysis"
        elif situation == "page_navigation":
            assert (
                text == PAGE_NAVIGATION_TEXT
            ), "Text does not match the expected text for page_navigation"
...
def handle_dialog(request: DialogRequest):
    ...
    category = judge_message_category(
        processor.get_first_k_messages_str_excluding_sys(1) + request.query
    )
    ...
    if category == "data_analysis":
        ...
        return result
    if (
        processor.function_descriptions != function_descriptions
        or processor.system_prompt != system_prompt
    ):
