...
def optimize_entry(entry, pyedit, department_codes, person_ids):
    ...
    if visited_person and visited_department:
        # 部门和受访人都做最近处理的优化
        relationship, closest_department, _, closest_person, _ = pyedit.get_relationship(visited_department, visited_person)
        ...
        relationship, closest_department, _, closest_person, _ = pyedit.get_relationship('', visited_person)
        # 只有受访人
        entry['visit_details']['visited_person'] = closest_person
        entry['id'] = person_ids.get(closest_person)
