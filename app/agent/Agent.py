...
class AutoProcessor:
    ...
    def update_system_prompt(self, new_system_prompt):
        ...
        if len(self.messages) == 0:  # 如果messages列表为空
            self.messages.append(SystemMessage(content=self.system_prompt))  # 添加一个新的系统消息
        else:
    ...
    def process(self, user_input):
        ...
        print(f"Return result from process: {result}")
        return result
    ...
    def get_first_k_messages_str_excluding_sys(self, k):
        return "\n".join([message.content for message in self.messages[-k:] if not isinstance(message, SystemMessage)])
